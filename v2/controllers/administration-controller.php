...
function admin_modifier_scene() {
    ...
    if (!admin_connecte()) redirection('403'); // VERIF Admin Connecté
...
function admin_modifier_scene_handler() {
    ...
    if ( // VERIFICATION des données postées
        ...
        ) redirection('admin-modifier-scene' . '&id=' . $_GET['id'], 'Informations postées manquantes ou invalides');
    ...
    if (verif_image() === false)
        redirection('admin-modifier-scene', 'Image Invalide, veuillez réessayer avec un format ou taille appropriées');
    ...
    // Affichage de la VUE
    redirection('episode&id=' . $_POST['id_episode'] . '&scene_id=' . $scene_trouve->id, 'La scène a bien été modifiée!#scn' . $_POST['numero']);
}
...
function admin_creer_scene_handler() {
    ...
    if ( // VERIFICATION des données postées
        ...
        || empty($_POST['temps']) || is_numeric($_POST['temps'])
        || empty($_POST['texte']) || is_numeric($_POST['texte'])
        ) redirection('admin-creer-scene', 'Informations postées manquantes ou invalides');
    
    $episode_parent = episode_trouve_par_id($_POST['id_episode']);
    $chapitre_parent = chapitre_trouve_par_id($episode_parent->id_chapitre);
    $saison_parent = saison_trouve_par_id($episode_parent->id_saison);

    // GESTION de l'image (qui est facultative donc la valeur null est autorisée)
    if (verif_image() === false)
        redirection('admin-creer-scene', 'Image Invalide, veuillez réessayer avec un format ou taille appropriées');
    elseif (verif_image() === null)
    ...
    // GESTION de la position / numero
    reordonner_fratrie(-1, $_POST['numero'], [], scenes_enfants_de_episode($_POST['id_episode']));
    ...
    $nouvelle_scene->id_episode = $_POST['id_episode'];;

    $nouvelle_scene->save();

    // AFFICHAGE de la VUE
    redirection('episode&id=' . $_POST['id_episode'] . '&scene_id=' . $nouvelle_scene->id, 'La scène a bien été créee!#scn' . $_POST['numero']);
...
function admin_supprimer_scene_handler() {
    ...
    else redirection('episode&id=' . $scene_trouve->id_episode, 'La scène a bien été supprimée!', '#tete-lecture');
}
...
function admin_modifier_episode_handler() {
    ...
    if ( // VERIFICATION des données postées
        ...
        ) redirection('admin-modifier-episode' . '&id=' . $_GET['id'], 'Informations postées manquantes ou invalides');
    ...
    // GESTION de l'image (qui est facultative donc la valeur null est autorisée)
    if (verif_image() === false)
        redirection('admin-modifier-episode', 'Image Invalide, veuillez réessayer avec un format ou taille appropriées');
    elseif (verif_image() === null)
    ...
    // Affichage de la VUE
    redirection('episode&id=' . $episode_trouve->id, 'L\'episode a bien été modifiée!#tete-lecture');
}
